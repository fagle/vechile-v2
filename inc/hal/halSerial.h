...
// * can test for.
#define EMBER_SERIAL_UNUSED    0x00
#define EMBER_SERIAL_FIFO      0x01
#define EMBER_SERIAL_BUFFER    0x02
#define EMBER_SERIAL_LOWLEVEL  0x03
...
typedef struct 
{
    int8u length;               // Number of bytes in this buffer to write.
    EmberMessageBuffer buffer;  // The first linked buffer.
    int8u startIndex;           // Starting position within the buffer.
...
typedef struct 
{
    int8u head;                        /** Index of next message to send */  
    int8u tail;                        /** Index of where to enqueue new messages.*/
    volatile int8u used;               /** Number of active messages.*/
    volatile int8u dead;               /** Number of messages sent that need cleanup.*/
    EmberMessageBuffer currentBuffer;  /** Packet buffer currently writing from, if any (used to step down a chain of buffers).*/
    int8u *nextByte;                   /** Pointer to the next byte to send from current packet buffer.*/
    int8u *lastByte;                   /** Pointer to the last byte to send from current packet buffer.*/
    EmSerialBufferQueueEntry fifo[];   /** FIFO of queued messages.*/
} EmSerialBufferQueue;

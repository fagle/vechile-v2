 ...
 * Default is to not enable TX_ACTIVE functionality on PC5, except when using
 * the EM35x as an NCP for EZSP-UART (since this Board Header file is shared
 * with that build).
 */
#ifdef EZSP_UART
  #define ENABLE_ALT_FUNCTION_TX_ACTIVE
#else
  // EM35x SoC applications wishing to use the TX_ACTIVE alternate function of
  // PC5 should uncomment the following line.
  //#define ENABLE_ALT_FUNCTION_TX_ACTIVE
#endif
 ...
 * Default is to not enable nTX_ACTIVE functionality on PC6, except when using
 * the EM35x as an NCP for EZSP-UART (since this Board Header file is shared
 * with that build).
 */
#ifdef EZSP_UART
...
#else
  // EM35x SoC applications wishing to use the nTX_ACTIVE alternate function of
  // PC6 should uncomment the following line.
  //#define ENABLE_ALT_FUNCTION_NTX_ACTIVE
#endif
...
#define POWERUP_SET_GPIO_CFG_REGISTERS()                           \
  GPIO_PACFGL = (GPIOCFG_OUT_ALT    <<PA0_CFG_BIT)|                \
                (GPIOCFG_IN         <<PA1_CFG_BIT)|                \
                (GPIOCFG_OUT_ALT    <<PA2_CFG_BIT)|                \
                (GPIOCFG_OUT        <<PA3_CFG_BIT);                \
  GPIO_PACFGH = (PWRUP_CFG_PTI_EN   <<PA4_CFG_BIT)|                \
  ...
  GPIO_PBCFGH = (GPIOCFG_OUT_ALT    <<PB4_CFG_BIT)|  /* SC1nRTS */ \
                (GPIOCFG_ANALOG     <<PB5_CFG_BIT)|                \
                (GPIOCFG_IN_PUD     <<PB6_CFG_BIT)|                \
                (GPIOCFG_OUT_ALT    <<PB7_CFG_BIT);                \
  GPIO_PCCFGL = (GPIOCFG_OUT_ALT    <<PC0_CFG_BIT)|                \

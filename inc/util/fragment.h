 ...
 * must return immediately from its emberIncomingMessageHandler(). 
 
 * @Returns FALSE if the incoming message was not part of a long message. The
 * fragmentation code has not handled the message so the application must
 * continue to process it. 
 
 * @Returns FALSE if the incoming message was a block that completed a long message. 
 * The fragmentation code replaces the message with the reassembled long message so the 
 * application must continue to process it
 */

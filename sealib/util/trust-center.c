...
/////////////////////////////////////////////////////////////////////////////////////////////
//
boolean trustCenterInit ( EmberKeyData* preconfiguredKey, EmberKeyData* networkKey )
{
    ...
    {
        sea_memcpy(emberKeyContents(&state.preconfiguredKey), preconfiguredKey, EMBER_ENCRYPTION_KEY_SIZE);
        // When using pre-configured TC link keys, devices are not allowed to request TC link keys. Otherwise it exposes a security hole.
    ...
    {
        sea_memcpy(emberKeyContents(&state.networkKey), networkKey, EMBER_ENCRYPTION_KEY_SIZE);
    }
...
#if defined EZSP_HOST
    ezspSetPolicy(EZSP_TRUST_CENTER_POLICY, (allow ? (trustCenterUsePreconfiguredKey
                                                   ? EZSP_ALLOW_PRECONFIGURED_KEY_JOINS
                                                   : EZSP_ALLOW_JOINS) : EZSP_ALLOW_REJOINS_ONLY));
#endif

                                       ...
                                       int8u maxFragmentSize )
{
    ...
    if (length == 0x00 || emberFragmentWindowSize == 0x00) 
        return EMBER_INVALID_CALL;
    totalTxBlocks = (length + maxFragmentSize - 0x01) / maxFragmentSize;
    FRAGMENT_DEBUG(simPrint("sending %d fragments to %04X", totalTxBlocks, indexOrDestination););

    abortTransmission(EMBER_ERR_FATAL);   // Clear out any existing traffic.
    MEMSET(txFragments, EMBER_NULL_MESSAGE_BUFFER, MAX_TOTAL_BLOCKS);
    ...
    {
        int8u i = totalTxBlocks - 0x01;
        int8u fragmentSize = length - ((totalTxBlocks - 0x01) * maxFragmentSize);
        int8u index = length;
    ...
    fragmentApsFrame.options |= (EMBER_APS_OPTION_FRAGMENT | EMBER_APS_OPTION_RETRY);
    txWindowBase    = 0x00;
    blocksInTransit = 0x00;
    {
...
#if 0    // last version, i think the function has some bugs about return true and false! 
                                   ...
                                   EmberStatus status )
{
    ...
    {
        FRAGMENT_DEBUG(simPrint("fragment %d sent", buffer););
        for (int8u i = 0x00; i < MAX_TOTAL_BLOCKS; i ++)
        {
            if (buffer == txFragments[i]) 
            {
                FRAGMENT_DEBUG(simPrint("have status %02X for fragment %d", status, i););
                if (status == EMBER_SUCCESS) 
                {
                    txFragments[i] = EMBER_NULL_MESSAGE_BUFFER;
                    emberReleaseMessageBuffer(buffer);
                    blocksInTransit -= 0x01;
                    if (blocksInTransit == 0x00) 
                    {
                        txWindowBase += emberFragmentWindowSize;
                        abortTransmission(sendNextFragments());
                    }
...
static EmberStatus sendNextFragments ( void )
{
    ...
    for (i = txWindowBase; (i - txWindowBase < emberFragmentWindowSize && i < totalTxBlocks); i ++) 
    {
        EmberStatus status;
    ...
    if (blocksInTransit == 0x00) 
    {
        FRAGMENT_DEBUG(simPrint("successfully sent %d fragments to %04X", totalTxBlocks, fragmentIndexOrDestination););
...
static void abortTransmission ( EmberStatus status )
{
    ...
    {
        FRAGMENT_DEBUG(simPrint("aborting %d fragments to %04X", totalTxBlocks, fragmentIndexOrDestination););
        releaseFragments(txFragments);
        fragmentOutgoingType = 0xFF;
...
static EmberEventControl abortReceptionEvent = {0, 0};    // An event for timing out failed incoming packets.

#define lowBitMask(n) ((1 << (n)) - 1)                    // A mask with the low n bits set.

static void setBlockMask ( void )
{
    int8u highestZeroBit = emberFragmentWindowSize;                  // Unused bits must be 1.
    
    if (rxWindowBase + emberFragmentWindowSize > expectedRxBlocks)   // If we are in the final window, there may be additional unused bits.
    {
...
boolean emberFragmentIncomingMessage ( EmberApsFrame *apsFrame, EmberMessageBuffer payload )
{
    ...
    else 
    {
        ...
        if (fragmentSource == EMBER_NULL_NODE_ID && blockNumber < emberFragmentWindowSize) 
        {
            ...
            setBlockMask();
            MEMSET(rxFragments, EMBER_NULL_MESSAGE_BUFFER, MAX_TOTAL_BLOCKS);
            FRAGMENT_DEBUG(simPrint("receiving fragments from %04X", fragmentSource););
            emberEventControlSetDelayMS(abortReceptionEvent, emberApsAckTimeoutMs * ZIGBEE_APSC_MAX_TRANSMIT_RETRIES);
        }

        if (!(emberGetSender() == fragmentSource && apsFrame->sequence == fragmentApsSequenceNumber))
            return TRUE;      // Drop unexpected fragments.
        else 
        {
            if (blockMask == 0xFF && rxWindowBase + emberFragmentWindowSize <= blockNumber) 
            {
            ...
            else 
            {
                ...
                if (!(mask & emMissedBlocks))         // Golden Unit Compliance
                {
                    FRAGMENT_DEBUG(simPrint("pretending to miss fragment %d (%04X)", blockNumber, apsFrame->groupId););
                    emMissedBlocks |= mask;           // clear the bit so a retransmission will succeed.
                    return TRUE;
                }                                     // End Compliance

                FRAGMENT_DEBUG(simPrint("have fragment %d (%04X)", blockNumber, apsFrame->groupId););

                if (blockNumber == 0x00) 
                {
                    expectedRxBlocks = HIGH_BYTE(apsFrame->groupId);
                    if (expectedRxBlocks < emberFragmentWindowSize) // Need to set unused bits in the window to 1. Previously a full window was assumed.
                        setBlockMask();
                    FRAGMENT_DEBUG(simPrint("%d fragments from %04X", expectedRxBlocks, fragmentSource););
                ...
                // ACK if it is the last block in the fragmented message, or it is a new message and we have all fragments 
                // in the window, or it is the last block in the window.
                if (blockNumber == expectedRxBlocks - 0x01 || (blockMask | lowBitMask(blockNumber % emberFragmentWindowSize)) == 0xFF) 
                {
                    FRAGMENT_DEBUG(simPrint("sending ack base %d mask %02X", rxWindowBase, blockMask););
                ...
                // To avoid the uncertain effects of copying from a buffer onto itself, we copy the payload into another buffer and 
                // then copy everything back into the original payload.
                if (blocksReceived == expectedRxBlocks) 
                {
                    ...
                    emberSetLinkedBuffersLength(payload, 0x00);

                    for (i = 0x00; i < expectedRxBlocks; i ++) 
                    {
                        EmberMessageBuffer temp = rxFragments[i];
                        int8u tempLength = emberMessageBufferLength(temp);

                        if (emberSetLinkedBuffersLength(payload, length + tempLength) != EMBER_SUCCESS)
                            goto kickout;
                    ...
                    FRAGMENT_DEBUG(simPrint("received %d fragments from %04X", expectedRxBlocks, fragmentSource););
...
static void abortReception ( void )
{
    ...
    {
        FRAGMENT_DEBUG(simPrint("aborting reception of %d fragments from %04X", expectedRxBlocks, fragmentSource););
        releaseFragments(rxFragments);
        fragmentSource = EMBER_NULL_NODE_ID;
    }
}

static EmberEventData fragmentationEvents[] = { {&abortReceptionEvent, abortReception}, {NULL, NULL} };      // terminator

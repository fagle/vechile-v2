...
#include "ember.h"
#include "error.h"
#include "ember-static-struct.h" // Required typedefs
#include "micro.h"
#include "network.h"
...
// the available space. On other platforms, we must allocate it here.
  ...
  VAR_AT_SEGMENT(int8u emAvailableMemory[END_stackMemory], __APP_RAM__);
...
#ifndef EMBER_APPLICATION_HAS_TRUST_CENTER_JOIN_HANDLER         // define in ledlamp-configuration.h 
...
{
    if (status == EMBER_STANDARD_SECURITY_SECURED_REJOIN || status == EMBER_DEVICE_LEFT || status == EMBER_HIGH_SECURITY_SECURED_REJOIN)
...
#ifndef EMBER_APPLICATION_HAS_CHILD_JOIN_HANDLER                 // define in ledlamp-configuration.h 
...
#ifndef EMBER_APPLICATION_HAS_MAC_PASSTHROUGH_HANDLER             // define in ledlamp-configuration.h 
void emberMacPassthroughMessageHandler(EmberMacPassthroughType messageType,
                                       EmberMessageBuffer message)
{
}
#endif
#ifndef EMBER_APPLICATION_HAS_RAW_HANDLER                         // define in ledlamp-configuration.h 
...
#ifndef EMBER_APPLICATION_HAS_ENERGY_SCAN_RESULT_HANDLER           // define in ledlamp-configuration.h 
...
#ifndef EMBER_APPLICATION_HAS_POLL_HANDLER                          // define in ledlamp-configuration.h 
...
#ifndef EMBER_APPLICATION_HAS_BUTTON_HANDLER                        // define in ledlamp-configuration.h 
void halButtonIsr(int8u button, int8u state)
{
}
#endif

#ifndef EMBER_APPLICATION_HAS_SOURCE_ROUTING                        // define in ledlamp-configuration.h 
...
{ 
    ...
    for (i = 0x00; i < emberEndpointCount; i++, endpoints++) 
    {
...
boolean emberMacPassthroughFilterHandler ( int8u *macHeader )
{
    ...
    for (u8 i = 0x00; i < EUI64_SIZE; i ++)
        Debug("%02x", macHeader[i]);
#endif    
    return FALSE;
}
#endif
...
void emberRadioNeedsCalibratingHandler(void)
{
    ...
    emberCalibrateCurrentChannel();   // TODO: Failsafe any critical processes or peripherals.
}
...
void halSimEepromCallback ( EmberStatus status )
{
    ...
    {
        // the Simulated EEPROM is full!  we must erase a page!  we should never reach this case if the PAGE_RED above calls ErasePage();
        halSimEepromErasePage();
        halSimEepromErasePage();
        halSimEepromErasePage();
        halSimEepromErasePage();
    } 
    else if ((status == EMBER_ERR_FLASH_WRITE_INHIBITED) || (status == EMBER_ERR_FLASH_VERIFY_FAILED)) 
    {
        // Something went wrong while writing a token.  There is stale data and the token the app expected to write did not get written. 
        // Also there is now "stray" data written in the flash that could inhibit future token writes. To deal with this stray data, 
        // we must repair the Simulated EEPROM.  Because the expected token write failed and will not be retried, it is best to reset 
        // the chip and let normal boot sequences take over. Since halInternalSimEeRepair() could potentially result in another write
    ...
    {
        assert(0);  // this condition indicates an unexpected problem.
    }
}

...
#include "ember-static-struct.h"
#endif //DOXYGEN_SHOULD_SKIP_THIS

//////////////////////////////////////////////////////////////////////////////////////////////////////////////
// @name Miscellaneous Ember Types
//
#define EUI64_SIZE                 8      // @brief Size of EUI64 (an IEEE address) in bytes (8).
#define EXTENDED_PAN_ID_SIZE       8      // @brief Size of an extended PAN identifier in bytes (8).
#define EMBER_ENCRYPTION_KEY_SIZE  16     // @brief Size of an encryption key in bytes (16).
#define EMBER_CERTIFICATE_SIZE     48     // @brief Size of Implicit Certificates used for Certificate Based Key Exchange.
#define EMBER_PUBLIC_KEY_SIZE      22     // @brief Size of Public Keys used in Eliptical Cryptography ECMQV algorithms.
#define EMBER_PRIVATE_KEY_SIZE     21     // @brief Size of Private Keys used in Eliptical Cryptography ECMQV alogrithms.
#define EMBER_SMAC_SIZE            16     // @brief Size of the SMAC used in Eliptical Cryptography ECMQV alogrithms.
#define EMBER_SIGNATURE_SIZE       42     // @brief Size of the DSA signature used in Eliptical Cryptography Digital Signature Algorithms. 

typedef int8u    EmberStatus;             // @brief  Return type for Ember functions.
typedef int8u    EmberEUI64[EUI64_SIZE];  // @brief EUI 64-bit ID (an IEEE address).
typedef int8u    EmberMessageBuffer;      // @brief Incoming and outgoing messages are stored in buffers. These buffers are allocated 
                                          // and freed as needed. Buffers are 32 bytes in length and can be linked together to hold
                                          // longer messages. See packet-buffer.h for APIs related to stack and linked buffers.
typedef int16u   EmberNodeId;             // @brief 16-bit ZigBee network address.
typedef int16u   EmberMulticastId;        // @brief 16-bit ZigBee multicast group identifier. 
typedef int16u   EmberPanId;              //  @brief 802.15.4 PAN ID. 

#define EMBER_MAX_802_15_4_CHANNEL_NUMBER (26)            // The maximum 802.15.4 channel number is 26.
#define EMBER_MIN_802_15_4_CHANNEL_NUMBER (11)            // The minimum 802.15.4 channel number is 11.
#define EMBER_NUM_802_15_4_CHANNELS       (EMBER_MAX_802_15_4_CHANNEL_NUMBER - EMBER_MIN_802_15_4_CHANNEL_NUMBER + 0x01)
#define EMBER_ALL_802_15_4_CHANNELS_MASK  0x07FFF800UL   // Bitmask to scan all 802.15.4 channels.
#define EMBER_ZIGBEE_COORDINATOR_ADDRESS  0x0000         // The network ID of the coordinator in a ZigBee network is 0x0000.

#define EMBER_NULL_NODE_ID                0xFFFF  // @brief A distinguished network ID that will never be assigned to any node.  Used to indicate the absence of a node ID.
#define EMBER_NULL_BINDING                0xFF    // @brief A distinguished binding index used to indicate the absence of a binding.
#define EMBER_TABLE_ENTRY_UNUSED_NODE_ID  0xFFFF  // @brief A distinguished network ID that will never be assigned to any node.  
                                                  // This value is used when setting or getting the remote node ID in the address table 
                                                  // or getting the remote node ID from the binding table.  It indicates that address 
                                                  // or binding table entry is not in use.
#define EMBER_MULTICAST_NODE_ID           0xFFFE  // @brief A distinguished network ID that will never be assigned to any node. This value 
                                                  // is returned when getting the remote node ID from the binding table and the given binding 
                                                  // table index refers to a mulicast binding entry.
#define EMBER_UNKNOWN_NODE_ID             0xFFFD  // @brief A distinguished network ID that will never be assigned to any node. This value 
                                                  // is used when getting the remote node ID from the address or binding tables. It indicates 
                                                  // that the address or binding table entry is currently in use but the node ID corresponding 
                                                  // to the EUI64 in the table is currently unknown.
#define EMBER_DISCOVERY_ACTIVE_NODE_ID    0xFFFC  // @brief A distinguished network ID that will never be assigned to any node. This value 
                                                  // is used when getting the remote node ID from the address or binding tables. It indicates 
                                                  // that the address or binding table entry is currently in use and network address discovery is underway.
#define EMBER_NULL_ADDRESS_TABLE_INDEX    0xFF    // @brief A distinguished address table index used to indicate the absence of an address table entry.
#define EMBER_ZDO_ENDPOINT                0x00    // @brief The endpoint where the ZigBee Device Object (ZDO) resides.
#define EMBER_BROADCAST_ENDPOINT          0xFF    // @brief The broadcast endpoint, as defined in the ZigBee spec.
#define EMBER_ZDO_PROFILE_ID              0x0000  // @brief The profile ID used by the ZigBee Device Object (ZDO).

//////////////////////////////////////////////////////////////////////////////////////////////////////////////
// @name ZigBee Broadcast Addresses
// ZigBee specifies three different broadcast addresses that reach different collections of nodes.  Broadcasts are normally sent only
// to routers.  Broadcasts can also be forwarded to end devices, either all of them or only those that do not sleep.  Broadcasting to end
// devices is both significantly more resource-intensive and significantly less reliable than broadcasting to routers.
//
#define EMBER_BROADCAST_ADDRESS                 0xFFFC    // Broadcast to all routers. 
#define EMBER_RX_ON_WHEN_IDLE_BROADCAST_ADDRESS 0xFFFD    // Broadcast to all non-sleepy devices. 
#define EMBER_SLEEPY_BROADCAST_ADDRESS          0xFFFF    // Broadcast to all devices, including sleepy end devices. 


//////////////////////////////////////////////////////////////////////////////////////////////////////////////
// @brief Defines the possible types of nodes and the roles that a node might play in a network.
//
...
#endif
{
  EMBER_UNKNOWN_DEVICE    = 0,  /** Will relay messages and can act as a parent to other nodes. */
  EMBER_COORDINATOR       = 1,  /** Will relay messages and can act as a parent to other nodes. */
  EMBER_ROUTER            = 2,  /** Communicates only with its parent and will not relay messages. */
  EMBER_END_DEVICE        = 3,  /** An end device whose radio can be turned off to save power.  The application must call ::emberPollForData() to receive messages. */
  EMBER_GATEWAY           = 4,
  EMBER_SLEEPY_END_DEVICE = 5,  /** A sleepy end device that can move through the network. */
  EMBER_MOBILE_END_DEVICE = 6
};
...
#endif
{
  ...
  EMBER_HIGH_SECURITY_UNSECURED_JOIN       = 5,     /* 6 Reserved */
  EMBER_HIGH_SECURITY_UNSECURED_REJOIN     = 7,     /* 8 - 15 Reserved */
};
...
typedef struct 
{
    int8u   extendedPanId[8];   // The network's extended PAN identifier.
    ...
    EmberJoinMethod joinMethod; // Join method: The protocol messages used to establish an initial parent. It is ignored 
                                // when forming a ZigBee network, or when querying the stack for its network parameters.
    EmberNodeId nwkManagerId;   // NWK Manager ID.  The ID of the network manager in the current network. This may only be set 
                                // at joining when using EMBER_USE_NWK_COMMISSIONING as the join method. 
    int8u nwkUpdateId;          // NWK Update ID.  The value of the ZigBee nwkUpdateId known by the stack. This is used to determine 
                                // the newest instance of the network after a PAN ID or channel change.  This may only be set at joining 
                                // when using EMBER_USE_NWK_COMMISSIONING as the join method.
    int32u channels;            // NWK channel mask.  The list of preferred channels that the NWK manager has told this device to use 
                                // when searching for the network. This may only be set at joining when using EMBER_USE_NWK_COMMISSIONING as the join method. 
} EmberNetworkParameters;


#ifdef DOXYGEN_SHOULD_SKIP_THIS
#define emberInitializeNetworkParameters(parameters)   (MEMSET(parameters, 0x00, sizeof(EmberNetworkParameters)))
#else
...
typedef struct 
{
    ...
    EmberApsOption options;      // A bitmask of options from the enumeration above. 
...
typedef struct 
{
    ...
    int16u clusterId;       // A cluster ID that matches one from the local endpoint's simple descriptor. This cluster ID is set by the 
                            // provisioning application to indicate which part an endpoint's functionality is bound to this particular 
                            // remote node and is used to distinguish between unicast and multicast bindings. Note that a binding can be 
                            // used to to send messages with any cluster ID, not just that listed in the binding.
    int8u remote;           // The endpoint on the remote node (specified by \c identifier). 
    EmberEUI64 identifier;  // A 64-bit identifier.  This is either: The destination EUI64, for unicasts A 16-bit multicast group address, for multicasts
...
typedef struct 
{
    ...
    int8u  averageLqi;  // An exponentially weighted moving average of the link quality values of incoming packets from this neighbor 
                        // as reported by the PHY.
    int8u  inCost;      // The incoming cost for this neighbor, computed from the average LQI. Values range from 1 for a good link to 
                        // 7 for a bad link.
    int8u  outCost;     // The outgoing cost for this neighbor, obtained from the most recently received neighbor exchange message 
                        // from the neighbor.  A value of zero means that a neighbor exchange message from the neighbor has not been
                        // received recently enough, or that our id was not present in the most recently received one.  EmberZNet Pro only.
    int8u  age;         // In EmberZNet Pro, the number of aging periods elapsed since a neighbor exchange message was last received from 
                        // this neighbor.  In stack profile 1, the number of aging periods since any packet was received. An entry with 
                        // an age greater than 3 is considered stale and may be reclaimed.  The aging period is 16 seconds.
...
typedef struct 
{
    int16u destination;       // The short id of the destination. 
    int16u nextHop;           // The short id of the next hop to this destination. 
    int8u  status;            // Indicates whether this entry is active (0), being discovered (1), or unused (3). 
    int8u  age;               // The number of seconds since this route entry was last used to send a packet. 
    int8u  concentratorType;  // Indicates whether this destination is a High RAM Concentrator (2), a Low RAM Concentrator (1),
                              // or not a concentrator (0). */
    int8u  routeRecordState;  // For a High RAM Concentrator, indicates whether a route record is needed (2), has been sent (1), 
                              // or is no long needed (0) because a source routed message from the concentrator has been received.
...
/** @brief This data structure contains the key data that is pasesd into various other functions. */
typedef struct 
{ 
  int8u contents[EMBER_ENCRYPTION_KEY_SIZE];    // This is the key byte data. 
} EmberKeyData;

/** @brief This data structure contains the certificate data that is used for Certificate Based Key Exchange (CBKE). */
typedef struct 
{
  int8u contents[EMBER_CERTIFICATE_SIZE];      // This is the certificate byte data. 
} EmberCertificateData;

/** @brief This data structure contains the public key data tha is used for Certificate Based Key Exchange (CBKE). */
typedef struct 
{
  int8u contents[EMBER_PUBLIC_KEY_SIZE];
} EmberPublicKeyData;

/** @brief This data structure contains the private key data tha is used for Certificate Based Key Exchange (CBKE). */
typedef struct 
{
  int8u contents[EMBER_PRIVATE_KEY_SIZE];
} EmberPrivateKeyData;

/** @brief This data structure contains the Shared Message Authentication Code (SMAC) data that is used for Certificate Based Key Exchange (CBKE). */
typedef struct 
{
  int8u contents[EMBER_SMAC_SIZE];
} EmberSmacData;

/** @brief This data structure contains a DSA signature.  It is the bit concatenantion of the 'r' and 's' components of the signature. */
typedef struct 
{
...
#endif
{
  ...
  /** This enables ZigBee High Security on the node. */
  ...
  */
  ...
  // This next bit is free.
  //  EM_FREE_BIT                           = 0x1000,

  EM_SAVED_IN_TOKEN                         = 0x4000,
  #define EM_SECURITY_INITIALIZED           0x00008000
#else 
...
#endif
{
  ...
  /** This denotes that the device is running in a network with ZigBee 
   *  High Security. */
  EMBER_HIGH_SECURITY_MODE_                 = 0x0001,

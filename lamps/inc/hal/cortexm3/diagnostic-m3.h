...
typedef struct
{
  int16u        bootPanId;        // PanId that the bootloader will use
  int8u         bootRadioChannel; // emberGetRadioChannel() - 11
  int8u         bootRadioPower;   // emberGetRadioPower()
  int8u         bootRadioCfCal;   // Channel Filter calibration
  int8u         bootRadioLnaCal;  // Low Noise Amplifier calibration
  int8u         bootReserved[19]; // (reserved for future use)
  int8u         bootMode;         // halLaunchStandaloneBootloader() mode arg
  int16u        bootEvent;        // CE_REBOOT_BOOTLOAD
  int16u        bootMagicHi;      // BootLoadMagicHi
  int16u        bootValid;        // 0xA11D marks BootParam Valid
                                  // also BootLoadMagicLo
} HalBootParamType;

// note that assert and dmaProt are written just before a forced reboot
typedef union
{
  struct { PGM_P file; int line; } assert;

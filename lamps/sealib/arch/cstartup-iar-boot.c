...
void checkAppJumpToApp(void)
{
  ...
  if( ((RESET_EVENT & RESET_SW_MASK) != RESET_SW_MASK) ||
      ( ( ((RESET_EVENT & RESET_SW_MASK) == RESET_SW_MASK) && 
          (halResetInfo.crash.resetSignature == RESET_VALID_SIGNATURE) 
        ) &&
    ...
    //FIXME -- This address table check should be beefed up before declaring
    //         the app is valid and jumping to the application.
...
{
  int32u *dataSourceStart;
  int32u *dataDestinationStart;
  int32u *dataDestinationEnd;
  ...
  while(dataDestinationStart < dataDestinationEnd) {
    ...
    *dataDestinationStart++ = STACK_FILL_VALUE;
  }
